        ...
        try {
          ...
          if (!response.ok) {
            ...
            if (contentType.includes('application/json')) {
              ...
              errorBody = textBody ? { rawText: textBody } : {};
            }

            const serverMessage = errorBody?.error || errorBody?.message || errorBody?.rawText || `${response.status} ${response.statusText}`;
          ...
          const contentType = response.headers.get('content-type') || '';
          if (!contentType.includes('application/json')) {
            lastError = new Error(`Unexpected content type from ${loginUrl}: ${contentType || 'unknown'}`);
            continue;
          }
          ...
          try {
            ...
            lastError = new Error(`Invalid JSON response from ${loginUrl}`);
            continue;
          }

          if (typeof data?.success !== 'boolean') {
            const fallbackMessage = data?.error || data?.message || 'Invalid login response';
            if (fallbackMessage) {
              lastError = new Error(String(fallbackMessage));
              continue;
            }
          }
